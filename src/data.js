export const servicesData = [
    {
      icon: "flaticon-nanotechnology",
      title: "Teaching Tech",
      text: "Empowering learners with technology education to stay ahead in the digital world."
    },
    {
      icon: "flaticon-profile",
      title: "Mobile Development",
      text: "Building user-friendly mobile applications for iOS and Android platforms."
    },
    {
      icon: "flaticon-web-development",
      title: "Web Development",
      text: "Creating responsive and dynamic websites tailored to your business needs."
    },
    {
      icon: "flaticon-strategy",
      title: "IT Support",
      text: "Get reliable tech support anytime to keep your systems running smoothly and efficiently."
    },
    {
      icon: "flaticon-search",
      title: "Technical Support",
      text: "Get reliable tech support anytime to keep your systems running smoothly and efficiently."
    },
    {
      icon: "flaticon-database",
      title: "Data Banking",
      text: "Securing and managing your data efficiently with our comprehensive data banking services."
    }
  ];
export const socialMediaLinks = [
    {
      title: "Facebook",
      url: "https://www.facebook.com/profile.php?id=61570145900964",
      iconClass: "fab fa-facebook-f",
    },
    {
      title: "LinkedIn",
      url: "https://www.linkedin.com/company/syntechtic-solutions/",
      iconClass: "fab fa-linkedin",
    },
    {
      title: "Instagram",
      url: "https://www.instagram.com/synt.echticsolutions/",
      iconClass: "fab fa-instagram",
    },
    {
      title: "X",
      url: "https://x.com/SynS_services?t=QZFW-mHsSz5dZ36_iR8QYg&s=09",
      iconClass: "fab fa-x-twitter",
    },
  ];
  export const clientLogos = [
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/amazonwebservices/amazonwebservices-original-wordmark.svg",
      alt: "AWS Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/android/android-original-wordmark.svg",
      alt: "Android Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/azuresqldatabase/azuresqldatabase-original.svg",
      alt: "Azure SQL Database Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/csharp/csharp-original.svg",
      alt: "C# Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/dart/dart-original-wordmark.svg",
      alt: "Dart Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/django/django-plain.svg",
      alt: "Django Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/dotnetcore/dotnetcore-plain.svg",
      alt: "DotNet Core Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/typescript/typescript-original.svg",
      alt: "DotNet Core Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/flask/flask-original.svg",
      alt: "DotNet Core Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nextjs/nextjs-original-wordmark.svg",
      alt: "DotNet Core Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/go/go-original.svg",
      alt: "DotNet Core Logo",
    },
    {
      src: "https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/java/java-original-wordmark.svg",
      alt: "DotNet Core Logo",
    },
  ];

export   const portfolioData = {
  heading: {
    preTitle: "Open Projects",
    title: "Awesome Software",
    linkText: "see more",
    linkHref: "portfolio-grid.html"
  },
  categories: [
    { name: "All", filter: "*" },
    { name: "Mobile Apps", filter: "mobile" },
    { name: "Website", filter: "web" },
    { name: "AI", filter: "data" },
    { name: "API's", filter: "api" }
  ],
  items: [
    {
      id: 1,
      category: "web",
      image: "/assets/images/project/DrKilinkSnaps/1.jpg",
      alt: "portfolio item photo",
      title: "Web Appliation",
      link: "https://github.com/Syntechtic-Solutions/-Dr-Klinik-AI-Detector",
      images: [
        '/assets/images/project/DrKilinkSnaps/1.jpg',
        '/assets/images/project/DrKilinkSnaps/2.jpg',
        '/assets/images/project/DrKilinkSnaps/3.jpg',
        '/assets/images/project/DrKilinkSnaps/4.jpg',
        '/assets/images/project/DrKilinkSnaps/5.jpg',
        '/assets/images/project/DrKilinkSnaps/6.jpg',
        '/assets/images/project/DrKilinkSnaps/7.jpg',
        '/assets/images/project/DrKilinkSnaps/8.jpg'
      ]
    },
    {
      id: 2,
      category: "web",
      image: "/assets/images/project/csc/1.jpg",
      alt: "portfolio item photo",
      title: "Web Application",
      link: "https://github.com/Syntechtic-Solutions/Land-Surf",
      images: [
        '/assets/images/project/csc/1.jpg',
        '/assets/images/project/csc/2.jpg',
        '/assets/images/project/csc/3.jpg',
        '/assets/images/project/csc/4.jpg',
        '/assets/images/project/csc/5.jpg',
        '/assets/images/project/csc/6.jpg',
        '/assets/images/project/csc/7.jpg',
        '/assets/images/project/csc/8.jpg'
      ]
    },
    {
      id: 3,
      category: "data",
      image: '/assets/images/project/bot/1.png',
      alt: "portfolio item photo",
      title: "data analysis",
      link: "https://github.com/Syntechtic-Solutions/Chatbot-llm",
      images: [
        '/assets/images/project/bot/1.png',
        '/assets/images/project/bot/2.png',
      ]
    },
    {
      id: 4,
      category: "api",
      image: '/assets/images/project/mra/1.png',
      alt: "portfolio item photo",
      title: "API",
      link: "https://github.com/Syntechtic-Solutions/MRAssistant",
      images: [
        '/assets/images/project/mra/1.png',
        '/assets/images/project/mra/2.png',
      ]
    },
    {
      id: 5,
      category: "data",
      image: '/assets/images/project/pred/2.png',
      alt: "portfolio item photo",
      title: "Artifical Intelligence",
      link: "https://github.com/Syntechtic-Solutions/Task-management-app",
      images: [
        '/assets/images/project/pred/1.png',
        '/assets/images/project/pred/2.png',
        '/assets/images/project/pred/3.png',
        '/assets/images/project/pred/4.png',
        '/assets/images/project/pred/5.png',
        '/assets/images/project/pred/6.png',
      ]
    },
    {
      id: 6,
      category: "data",
      image: "/assets/images/project/whatapp bot/4.jpeg",
      alt: "portfolio item photo",
      title: "Artifical Intelligence",
      link: "https://github.com/Syntechtic-Solutions/whatsapp-bot-gemini-student-handbook",
      images: [
        '/assets/images/project/whatapp bot/1.jpeg',
        '/assets/images/project/whatapp bot/2.jpeg',
        '/assets/images/project/whatapp bot/3.jpeg',
        '/assets/images/project/whatapp bot/4.jpeg',
        '/assets/images/project/whatapp bot/5.jpeg',
        '/assets/images/project/whatapp bot/6.jpeg',
        '/assets/images/project/whatapp bot/7.jpeg',

      ]
    }
  ]
};
export const teamMembers = [
  {
    name: "Ezeana Micheal",
    role: "CTO",
    image: "assets/images/our-team/mic.jpg",
  },
  {
    name: "Ahmeed Abdul-Jalal",
    role: "CEO",
    image: "assets/images/our-team/jal.jpg",
  },
  {
    name: "Osuoha Nnamdi",
    role: "COO",
    image: "assets/images/our-team/nna.jpg",
  },
];